...
global mobile, labels, mobile_x, mobile_y, text, canvas, mobile_list, root, num_nodes, tf1, nodes, optimal_cluster, cluster_label, ieeecp, eecp, redundant
option = 0
energy = {}
cluster_head = []
...
def generateWSN():
    global mobile, labels, mobile_x, mobile_y, num_nodes, tf1, nodes, ieeecp, eecp, redundant
    ieeecp = 0
    redundant = 0
    eecp = 0
    mobile = []
...
def modifiedFCM():
    ...
    if num_nodes > 25 and optimal_cluster < cluster_th: #if cluster optimal size < cluster threshold then call Modified FCM to create clusters based on nearest neigbors
        cluster_label, selected_cent = Cluster.modifiedFCM(nodes, 50, optimal_cluster, centers) #here calling modified FCM to rearrnage cluster
...
def sendPacket():
    text.delete('1.0', END)
    global energy, ieeecp, eecp, redundant
    ...
    dup = []
    for i in range(0, 20):
        duplicate = random.randint(15, 45)
        if duplicate not in dup:
    ...
    for i in range(len(nodes)):
        ...
        if i != src and i != cluster_id and temp_x < cls_x:
            ...
            if dist < distance and dist1 < distance1:
                ...
                ieeecp = (distance * 0.5)                  
    if hop != -1:
        redundant = ieeecp - ((ieeecp / 100) * redundant)
        temp = nodes[hop]
        hop_x = temp[0]
        hop_y = temp[1]
...
def graph():
    global ieeecp, eecp, redundant
    height = [ieeecp, eecp]
    bars = ('Energy without Attack','Energy with Attack')
...
def Main():
    ...
    mfcmButton = Button(root, text="Cluster WSN", command=modifiedFCM)
    mfcmButton.place(x=820,y=210)
    mfcmButton.config(font=font1)

    chselectButton = Button(root, text="Cluster Head Selection", command=CHSelection)
